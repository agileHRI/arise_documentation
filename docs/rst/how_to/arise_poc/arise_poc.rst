...
.. image:: images/ARISE-Schema-v2.png
   :alt: ARISE PoC Schema
   :align: center
...
**ROS 2 and Fast DDS**
  ...
  More information can be found in `Documentation <https://fast-dds.doc.eprosima.com/en/latest/>`__ and in `GitHub <https://github.com/FIWARE/context.Orion-LD/tree/develop/demo/ros2>`__
...
**Orion LD Context Broker**
  ...
  More information can be found in `Documentation <https://github.com/FIWARE/context.Orion-LD/tree/develop/doc/manuals-ld>`__ and in `GitHub <https://github.com/FIWARE/context.Orion-LD>`__
...
.. image:: images/ARISE_PoC_Container.png
   :alt: Container view
   :align: center

Before starting the ROS 2 demo, as X11 session owners, other users must be allowed to use the X Window System (to show the turtles on screen).
...
.. image:: images/keyboard_turtlesim.png
   :alt: turtlesim view
   :align: center
...
In the context of the ARISE PoC, Grafana is used to visualize and analyze the data collected from OPC UA and ROS 2 devices, offering an intuitive interface to monitor the performance, and trends of connected systems.
...
- Construct your query to fetch the desired data, selecting the appropriate visualization type (e.g., graph, table, gauge) from the options.
  More information on how to structure queries useful for creating dashboards can be found in the `Example Query Dashboard`_ section of the present tutorial

  .. image:: images/insert_query.png
     :alt: Insert Query
     :align: center

  .. image:: images/configure_graph_type.png
     :alt: Selecting the appropriate visualization type
     :align: center
...
.. image:: images/Alerting_menu.png
   :alt: Select Alerting/Contact Points menu
   :align: center

.. image:: images/contact_point.png
   :alt: Define Contact Points
   :align: center
...
.. image:: images/dashboard_alert_rule.png
   :alt: Dashboard Alert Rule
   :align: center

Click on "New Alert Rule" to define a new Rule, enter the name, define the query and alert condition,

.. image:: images/newrule_1.png
   :alt: Query and Alert Condition
   :align: center
...
.. image:: images/ARISE_AlertNotification.jpg
   :alt: ARISE Alert Notification
   :align: center
   :width: 300
...
For the IoT Agent OPC UA to work an **initialization** phase is required. During this phase the IoT Agent becomes aware of what variables and methods are available on OPC UA server-side.
These information can be provided to the agent by means of a configuration file (``config.js``) or through the REST API.
...
.. literalinclude:: resources/mapping_tool.json
   :language: json
...
Docker Compose can be downloaded here `docker-compose.yaml <https://github.com/Engineering-Research-and-Development/arise-poc/blob/main/docker-compose.yaml>`__:
...
The ``entityId`` column identifies the ID of the entity, while the ``id`` column specifies the type of the data, such as ``Pose``.
The data are described in the ``compound`` column, which is defined as a ``jsonb`` type.
To access the data contained in the ``compound`` column, it can use the functions provided by TimescaleDB.
For example, writing ``a.compound->>'x'`` can access the value that this variable takes on in the composite column, i.e., ``x = 9.943054``.
